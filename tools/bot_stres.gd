...
func _calistir() -> void:
	...
	for t in tohumlar:
		...
		for kare in 60 * sure:
			...
			var k := int(round((x - kaydir - oyun._izgara0) / oyun._adim))
			var kx: float = oyun._izgara0 + k * oyun._adim
			...
			for p in oyun.parcalar:
				...
				if x >= p.position.x - 20 and x <= p.position.x + p.uzunluk:
					...
					if p.has_meta("desenler"):
						var giris: float = p.uzunluk - Ritim.PARCA_OLCU * Ritim.OLCU * oyun._adim
						var olcu := clampi(int(floor((yer - giris) / (Ritim.OLCU * oyun._adim))), 0, Ritim.PARCA_OLCU - 1)
						ad = "ritim[%s] ölçüde %.0f px" % [str(p.get_meta("desenler")[olcu]), yer - giris - olcu * Ritim.OLCU * oyun._adim]
